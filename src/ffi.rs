...
// and matches the FULL C layout of OQS_KEM and OQS_SIG so length fields are correct.
...
extern "C" {
    ...
    fn OQS_SIG_verify(
        ...
        msg_len: usize,
        sig_in: *const u8,
        sig_len: usize, // <-- FIX: include signature length
        pub_key: *const u8,
    ) -> c_int;
}
...
struct OQS_KEM {
    ...
    claimed_nist_level: c_uint, // uint8_t in C; use c_uint for stable ABI
    ind_cca: c_uint,            // uint8_t/bool-ish
    ...
    // function pointers (opaque for layout)
...
struct OQS_SIG {
    ...
    method_name: *const c_char,
    alg_version: *const c_char,
    claimed_nist_level: c_uint, // uint8_t
    euf_cma: c_uint,            // bool
...
unsafe fn sig_new_with_fallback() -> *mut OQS_SIG {
    // Common variants across liboqs builds
...
pub fn dilithium2_verify(pk: &[u8], msg: &[u8], sig_in: &[u8]) -> Result<(), OqsError> {
    unsafe {
        ...
        let rc = OQS_SIG_verify(
            ...
            msg.len(),
            sig_in.as_ptr(),
            sig_in.len(), // <-- FIX: pass signature length
            pk.as_ptr(),
        );
        OQS_SIG_free(sig);
        if rc != 0 {
